...
class PaymentMethod {
            ...
            <div class="payment-details">
                <div class="payment-primary">
					...
					<?php if ( $this->payment['status'] === 'expired' ) : ?>
                        <span class="payment-badge expired">Expired</span>
					<?php endif; ?>
                </div>
				...
				<?php if ( $this->config['format'] === 'detailed' ) : ?>
					...
					<?php if ( ! empty( $this->payment['provider'] ) ) : ?>
                        <div class="payment-provider">
                            via <?php echo esc_html( ucfirst( $this->payment['provider'] ) ); ?>
                        </div>
					<?php endif; ?>
				<?php endif; ?>
            </div>
	...
	private function get_payment_display(): string {
		...
		if ( $this->payment['type'] === 'bank' && ! empty( $this->payment['last4'] ) ) {
			return '<span class="payment-bank">Bank Account •••• ' . $this->payment['last4'] . '</span>';
		}

		if ( $this->payment['type'] === 'paypal' && ! empty( $this->payment['email'] ) ) {
			return '<span class="payment-paypal">PayPal (' . $this->payment['email'] . ')</span>';
		}
