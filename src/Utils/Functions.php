...
use ArrayPress\WPFlyout\Registry;
...
if ( ! function_exists( 'get_flyout_manager' ) ) {
	/**
	 ...
	 *
	 */
	function get_flyout_manager( string $prefix ): ?Manager {
		try {
			$registry = Registry::get_instance();
			...
			error_log( sprintf(
				'WP Flyout: Failed to get manager for prefix "%s" - %s',
				$prefix,
				$e->getMessage()
			) );
...
if ( ! function_exists( 'flyout_manager_exists' ) ) {
	/**
	 * Check if a flyout manager exists
	 *
	 ...
	 *
	 */
	function flyout_manager_exists( string $prefix ): bool {
		$registry = Registry::get_instance();

		return $registry->has_manager( $prefix );
	}
}

if ( ! function_exists( 'get_all_flyout_managers' ) ) {
	/**
	 * Get all registered flyout managers
	 *
	 * Returns an array of all registered manager instances.
	 ...
	 *
	 */
	function get_all_flyout_managers(): array {
		$registry = Registry::get_instance();

		return $registry->get_all_managers();
	}
}
